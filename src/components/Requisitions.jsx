...
const Requisitions = () => {
  ...
  const handleSubmit = async (id, requisitionNumber) => {
    const confirmed = await showConfirmation({
      ...
      message: `Are you sure you want to submit requisition "${requisitionNumber}" for approval? You won't be able to edit it after submission.`,
      confirmText: 'Submit',
      cancelText: 'Cancel',
      type: 'default',
      icon: Send
  ...
  const handleApprove = async (id, requisitionNumber, action, comments = '') => {
    const actionText = action === 'approve' ? 'approve' : 'reject';
    const confirmed = await showConfirmation({
      title: `${actionText.charAt(0).toUpperCase() + actionText.slice(1)} Requisition`,
      message: `Are you sure you want to ${actionText} requisition "${requisitionNumber}"?`,
    ...
    if (confirmed) {
      try {
        await axios.post(`/api/requisitions/${id}/approve`, { action, comments });
        toast.success(`Requisition ${actionText}d successfully`);
        fetchRequisitions();
        fetchStats();
      } catch (error) {
        toast.error(error.response?.data?.error || `Error ${actionText}ing requisition`);
      }
    }
  };
  ...
  const canEdit = (requisition) => {
    return (requisition.requested_by === user.id && requisition.status === 'draft') ||
           user.role === 'admin';
  };

  const canDelete = (requisition) => {
    return (requisition.requested_by === user.id && requisition.status === 'draft') ||
           user.role === 'admin';
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-2">Manage material and resource requests</p>
        </div>
                ...
                <tr key={requisition.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    ...
                    <div className="text-sm text-gray-500">
                      ${requisition.total_estimated_cost?.toFixed(2) || '0.00'}
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="text-sm text-gray-900">
                    ...
                    {requisition.required_date && (
                      <div className="text-sm text-gray-500">
                        Due: {new Date(requisition.required_date).toLocaleDateString()}
                      </div>
                    )}
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex space-x-2">
                      ...
                      {requisition.status === 'draft' && requisition.requested_by === user.id && (
                        <button
                      ...
                      {canApprove(requisition) && (
                        <>
                          <button
                            onClick={() => handleApprove(requisition.id, requisition.requisition_number, 'approve')}
                            className="text-green-600 hover:text-green-900"
                          ...
                          <button
                            onClick={() => handleApprove(requisition.id, requisition.requisition_number, 'reject')}
